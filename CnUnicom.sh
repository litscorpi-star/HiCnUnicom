...
[[ $# != 0 ]] && all_parameter=($(echo $@)) || { echo 'Err  !!! Useage: bash this_script.sh membercenter 17620500815@130104'; exit 1; }
all_username_password=($(echo ${all_parameter[*]} | grep -oE "[0-9]{11}@[0-9]{6}"| sort -u | tr "\n" " "))

# 登录失败尝试修改以下这个appId的值为抓包获取的登录过的联通app,也可使用传入参数 appId@*************
appId=1f7af72ad6912d306b5053abf90c7ebb44a1f6cb612f804c8bf61f20eea97ee1f662dc965a4feec4f79a941ece7090c6d8e481acdf8be27feac8ecf75f276d43
